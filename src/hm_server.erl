...
-export([start_link/0]).
-export([login/2, logout/1, send/2, get_channel_log/2, delete_channel/1]).
...
% state will hold bidirectional mapping between id <-> pid
% and also contain the last messages sent in a channel in a list.
-record(state, {pid2channel, channel2pid, channels}).
...
handle_call({send, Channel, Msg}, _From, State) when is_binary(Msg) ->
	...
	case ets:lookup(State#state.channels, Channel) of
		[] ->
			ets:insert(State#state.channels, {Channel, [Msg]});
		[{_Channel, Log}] ->
			ets:insert(State#state.channels, {Channel, lists:sublist([Msg | Log], ?MAXLOGSIZE)})
	end,
	{reply, ok, State};
